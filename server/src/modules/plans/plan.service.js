...
export async function updatePlan(id, data) {
    ...
    const existing = await getSellerPlanById(planId);
    if (!existing) {
    ...
    return updateSellerPlan(planId, updateData);
}
// =================================
...
export async function fetchUserPlan(userId) {
    ...
    // ---------------------------------
    const existingPlan = await getUserPlan(normalizedUserId);
    if (existingPlan) {
        return existingPlan;
    }
    // ---------------------------------
    ...
    const freePlan = plans.find((plan) => plan.name
        .trim()
        .toUpperCase() ===
        "FREE");
    if (!freePlan) {
        throw new Error("FREE seller plan not found");
    }
    // ---------------------------------
