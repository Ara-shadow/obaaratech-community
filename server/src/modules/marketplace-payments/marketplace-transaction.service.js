...
export async function createMarketplaceTransaction(data) {
    validateMoney(data.amount, "Transaction amount");
...
// =====================================
export async function getMarketplaceTransactionByReference(transactionReference) {
    return prisma.marketplaceTransaction.findFirst({
        where: {
...
// =====================================
export async function getMarketplaceTransactionByProviderReference(providerReference) {
    return prisma.marketplaceTransaction.findFirst({
        where: {
            providerReference,
        },
...
export async function createSellerEarning(data) {
    validateMoney(data.grossAmount, "Gross amount");
    validateMoney(data.commissionAmount, "Commission amount");
    validateMoney(data.paymentFee ?? 0, "Payment fee");
    validateMoney(data.netAmount, "Net amount");
    if (data.commissionAmount >
        data.grossAmount) {
        throw new Error("Commission amount cannot exceed gross amount");
    }
    const expectedNetAmount = data.grossAmount -
        data.commissionAmount -
        (data.paymentFee ?? 0);
    if (data.netAmount !==
        expectedNetAmount) {
        throw new Error("Seller net amount does not match gross amount, commission and payment fee");
    }
...
export async function creditSellerPendingBalance(sellerId, grossAmount, commissionAmount, tx) {
    validateMoney(grossAmount, "Gross amount");
    validateMoney(commissionAmount, "Commission amount");
    if (commissionAmount >
        grossAmount) {
        throw new Error("Commission amount cannot exceed gross amount");
    }
...
export async function getSellerEarningByOrderItem(orderItemId) {
     ...
     * IMPORTANT:
     *
     * orderItemId is NOT marked @unique
     * in the Prisma schema.
     *
     * Therefore findUnique() would be invalid.
     */
...
// =====================================
// FIND EARNING BY TRANSACTION AND ITEM
// =====================================
export async function getSellerEarningByTransactionAndOrderItem(transactionId, orderItemId) {
    return prisma.sellerEarning.findFirst({
        where: {
            transactionId,
            orderItemId,
