...
async function getCommissionConfiguration(
    ...
    const settings =
        await tx.marketplaceFinancialSetting.findFirst({
    ...
    return {
        ...
        freeCommissionRate:
            Number(
                settings.freeCommissionRate
            ),

        premiumCommissionRate:
            Number(
                settings.premiumCommissionRate
            ),

        businessCommissionRate:
            Number(
                settings.businessCommissionRate
            )
...
async function getSellerCommissionRate(
    sellerId: string,
    tx: Prisma.TransactionClient
): Promise<number> {

    const configuration =
        await getCommissionConfiguration(
            tx
        );
...
function validateCommissionRate(
    ...
    if (
        ...
        throw new Error(
            "Invalid marketplace commission configuration"
        );
...
export async function initializeMarketplacePayment(
    ...
    if (
        ...
        throw new Error(
            "This marketplace currently supports Flutterwave online payments"
        );
    ...
    if (
        ...
        throw new Error(
            "This order payment session has expired"
        );
    ...
    if (
        ...
        throw new Error(
            "Marketplace payments currently support NGN only"
        );

    }

    let transaction =
        order.marketplaceTransaction;
    ...
    if (
        ...
        throw new Error(
            "This marketplace transaction has already been completed"
        );
    ...
    if (
        ...
        throw new Error(
            "Marketplace transaction amount does not match the order total"
        );
    ...
    await updateMarketplaceTransaction(
...
if (
    ...
    if (
        ...
        throw new Error(
            "Payment amount does not match the marketplace order amount"
        );
    ...
    const completed =
        await runMarketplacePaymentTransaction(

            async (tx) => {

                const currentTransaction =
                    await tx.marketplaceTransaction.findUnique({
                ...
                if (!currentTransaction) {
                    ...
                    throw new Error(
                        "Marketplace transaction no longer exists"
                    );
                ...
                if (
                    currentTransaction.order.paymentStatus ===
                    "PAID"
                ) {
                ...
                if (
                    currentTransaction.order.payment
                ) {
                ...
                for (
                    ...
                    await createSellerLedgerEntry(

                        {
                            ...
                            description:
                                `Marketplace commission (${commissionRate}%) for ${item.title}`
...
export async function fetchMarketplaceTransaction(
    transactionId: string,
    userId: string
) {
...
export async function fetchMarketplaceTransactionByOrder(
    orderId: string,
    userId: string
) {

    const transaction =
        await getMarketplaceTransactionByOrderId(
            orderId
        );
