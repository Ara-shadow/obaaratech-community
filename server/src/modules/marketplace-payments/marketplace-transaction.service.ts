...
export async function getMarketplaceTransactionByReference(
    transactionReference: string
) {
...
export async function getMarketplaceTransactionByProviderReference(
    providerReference: string
) {
    return prisma.marketplaceTransaction.findFirst({
        where: {
...
export async function createSellerEarning(
    ...
    if (
        ...
        throw new Error(
            "Commission amount cannot exceed gross amount"
        );
    }
    ...
    if (
        ...
        throw new Error(
            "Seller net amount does not match gross amount, commission and payment fee"
        );
    }
...
export async function createSellerLedgerEntry(
    ...
    if (!Number.isInteger(data.amount)) {
        throw new Error(
            "Ledger amount must be an integer"
        );
    }
...
export async function getSellerEarningByTransactionAndOrderItem(
    transactionId: string,
