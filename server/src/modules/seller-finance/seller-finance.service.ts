...
function getUserId(requestUser: { id?: string; userId?: string }) {
...
export async function listSellerSettlements(userId: string) {
...
export async function updateMarketplaceFinancialSettings(
    input: Partial<{
...
export async function reviewSettlement(
    settlementId: string,
    status: Extract<SettlementStatus, "COMPLETED" | "FAILED" | "CANCELLED">,
...
export function authenticatedUserId(user: { id?: string; userId?: string }) {
    return getUserId(user);
}

export type SettlementTransaction = Prisma.TransactionClient;
