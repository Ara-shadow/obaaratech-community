...
.superRefine((data, ctx) => {
    ...
    if (!data.openingTime) {
        ...
        ctx.addIssue({
            ...
            path: ["openingTime"],
            message:
                "Opening time is required when the seller is open"
        });
    ...
    if (!data.closingTime) {
        ...
        ctx.addIssue({
            ...
            path: ["closingTime"],
            message:
                "Closing time is required when the seller is open"
        });
...
export type SellerBusinessHourInput =
    z.infer<typeof sellerBusinessHourSchema>;

export type SellerBusinessHoursInput =
    z.infer<typeof sellerBusinessHoursSchema>;