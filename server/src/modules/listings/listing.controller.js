import { createNewListing, fetchListings, fetchListingById, fetchMyListings, editListing, removeListing, fetchSearchListings, updateListingStatus, fetchRelatedListings } from "./listing.service.js";
// ===============================
// CREATE LISTING
// ===============================
...
// RELATED LISTINGS
// ===============================
export async function getRelatedListingsController(request, reply) {
    const params = request.params;
    const listing = await fetchListingById(params.id);
    if (!listing) {
        return reply.code(404).send({
    ...
    const listings = await fetchRelatedListings(params.id, listing.categoryId, listing.type, listing.location);
    return reply.send({
        success: true,
        listings
    });
